...
def get_fee_tools(school_id: str):
    ...
    def get_fee_status(student_id: str = "") -> dict:
        try:
            ...
            return {"success": True, "student_id": sid, "total_fees": total_fees, "paid_amount": paid_amount, "pending_amount": pending_amount, "fees": fees, "payments": payments}
    ...
    def create_razorpay_link(student_id: str, amount: float, description: str = "School Fee Payment") -> dict:
        try:
            ...
            link_data = {"amount": int(amount*100), "currency": "INR", "accept_partial": False, "description": description, "customer": {"name": student.get("name", "Student"), "contact": student.get("phone", ""), "email": student.get("email", "")}, "notify": {"sms": True, "email": True}, "reminder_enable": True, "notes": {"school_id": school_id, "student_id": student_id, "reference_id": reference_id}}
            resp = client.payment_link.create(link_data)
            supabase.table("payments").insert({"school_id": school_id, "student_id": student_id, "amount": amount, "status": "pending", "reference_id": reference_id, "razorpay_link_id": resp.get("id")}).execute()
            return {"success": True, "payment_link": resp.get("short_url"), "link_id": resp.get("id"), "reference_id": reference_id, "amount": amount}
        except ImportError:
