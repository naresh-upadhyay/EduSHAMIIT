                  ...
                  child: SafeArea(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: () {
                            if (p.avatarUrl != null &&
                              ...
                              ImagePreviewDialog.show(context, p.avatarUrl!,
                                  title: p.name);
                            }
                          },
                          child: Stack(
                        ...
                        Text(
                          [
                            if (p.className.isNotEmpty) 'Class ${p.className}',
                            if (p.rollNumber.isNotEmpty)
                              'Roll No. ${p.rollNumber}',
                            if (p.session.isNotEmpty) 'Session ${p.session}'
                          ].join(' · '),
                          style: const TextStyle(
          ...
          onTap: () {
            if (status != 'missing' && uploadedMap.containsKey(name)) {
              ...
              if (lowUrl.contains('.jpg') ||
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content:
                        Text('📄 File is not an image (PDF/Doc)'.tr(ref))));
              }
            }
          },
          child: Container(
...
class _EditSheetState extends ConsumerState<_EditSheet> {
  ...
  Future<void> _save() async {
    ...
    if (_uploadedFileBytes != null && _uploadedFileName != null) {
      await ref
          .read(profileProvider.notifier)
          .uploadDocument(_uploadedFileBytes!, _uploadedFileName!, _docType);
    }
                      ...
                      children: [
                        ...
                        ...widget.profile.documents.map((doc) {
                          ...
                          return Padding(
                            ...
                            child: Text('$icon ${doc.type} (${doc.status})',
                                style: TextStyle(
                                    fontSize: 10,
                                    color: color,
                                    fontWeight: FontWeight.w600)),
  ...
  Widget _drop(String label, String val, List<String> items,
