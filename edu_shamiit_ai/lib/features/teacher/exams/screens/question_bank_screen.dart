...
class _QuestionBankScreenState extends ConsumerState<QuestionBankScreen> {
  ...
  Future<void> _exportTemplate(String type) async {
    try {
      ...
      final url = Uri.parse('${_apiService.baseUrl}/teacher/question-bank/export-template?type=${type.toLowerCase()}&subject=${Uri.encodeComponent(subject)}');
  ...
  Future<void> _handleBulkUpload(String type) async {
    try {
      ...
      if (result != null && result.files.first.bytes != null) {
        ...
        final count = await _apiService.bulkUploadQuestions(csvContent, subject, type.toLowerCase());
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to upload questions: $e'), backgroundColor: Colors.redAccent),
      );
    }
  }
            ...
            Expanded(
              child: _isLoading
                  ...
                  : _error != null
                      ...
                      : _questions.isEmpty
                          ? const Center(child: Text('No questions in repository. Click "+" to add.'))
                          : Center(
  ...
  Widget _buildFilterBar() {
    final List<String> subjectOptions = ['All', ..._subjects.map((s) => s.name).toSet()];
        ...
        children: [
          Expanded(
            child: DropdownButtonFormField<String>(
              ...
              decoration: const InputDecoration(labelText: 'Subject', contentPadding: EdgeInsets.symmetric(horizontal: 10)),
          ...
          Expanded(
            child: DropdownButtonFormField<String>(
              ...
              decoration: const InputDecoration(labelText: 'Difficulty', contentPadding: EdgeInsets.symmetric(horizontal: 10)),
            ...
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: levelColor.withOpacity(0.15), borderRadius: BorderRadius.circular(8)),
                  child: Text(
                    question.difficulty,
  ...
  Future<void> _confirmDeleteQuestion(String id) async {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete this question from the Question Bank?'),
        actions: [
  ...
  void _showQuestionFormDialog({QuestionBankItem? question}) {
    ...
    final qTextController = TextEditingController(text: question?.questionText ?? '');
    final chapterController = TextEditingController(text: question?.chapter ?? '');
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  const SizedBox(width: 12),
                                  Text(
                                    question != null ? 'Edit Question' : 'Add Question to Bank',
                                  ...
                                  children: [
                                    DropdownButtonFormField<String>(
                                      ...
                                      items: subjectNames
                                          ...
                                          .toList(),
                                      onChanged: (v) => setDialogState(() => selectedSubName = v!),
                                    ),
                                      ...
                                      children: [
                                        ...
                                        Expanded(
                                          child: TextFormField(
                                            ...
                                            validator: (v) => v == null || int.tryParse(v) == null ? 'Must be integer' : null,
                                          ),
                                        ),
                                      ],
                                    ),
                                    ...
                                    child: ElevatedButton(
                                      onPressed: () async {
                                        if (formKey.currentState!.validate()) {
                                          ...
                                          try {
                                            if (question != null) {
                                              await _apiService.updateQuestionInBank(question.id, body);
                                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('🎉 Question updated successfully!')));
                                            } else {
                                              await _apiService.addQuestionToBank(body);
                                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('🎉 Question added to bank!')));
                                            }
                                            _loadData();
                                          } catch (e) {
                                            setState(() {
  ...
  void _showBulkImportExportPanel() {
    ...
    showGeneralDialog(
      ...
      transitionBuilder: (context, anim1, anim2, child) {
        final isDesktop = MediaQuery.of(context).size.width >= 800;
        final double width = isDesktop ? 500 : MediaQuery.of(context).size.width;
        final beginOffset = isDesktop ? const Offset(1, 0) : const Offset(0, 1);
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  InkWell(
                                    onTap: () async {
                                      Navigator.pop(context); // Close panel before picking file
                                      ...
                                      child: Column(
                                        children: [
                                          Container(
                                            ...
                                            child: const Icon(Icons.file_upload_outlined, color: Color(0xFF6366F1), size: 28),
                                          ),
                                          const SizedBox(height: 12),
                                          const Text(
