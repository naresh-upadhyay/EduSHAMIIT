...
class ExamPaperBuilderScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ExamPaperBuilderScreen> createState() =>
      _ExamPaperBuilderScreenState();
}

class _ExamPaperBuilderScreenState
    extends ConsumerState<ExamPaperBuilderScreen> {
  ...
  int get _calculatedTotalMarks {
    return _examQuestions.fold<int>(0,
        (sum, q) => sum + (int.tryParse(q['marks']?.toString() ?? '1') ?? 1));
  }
          ...
          child: ElevatedButton(
            onPressed: () {
              context.pushReplacement(
                  '/teacher/exams/paper-builder?examId=${exam.id}');
            },
  ...
  Widget _buildExamSelectorScreen() {
    final classes = _allExams
        .map((e) => e.class_)
        .where((c) => c.isNotEmpty)
        .toSet()
        .toList()
      ..sort();
    final subjects = _allExams
        .map((e) => e.subject)
        .where((s) => s.isNotEmpty)
        .toSet()
        .toList()
      ..sort();
    final categories = _allExams
  ...
  Widget _buildMarksSummaryBar(int current, int target, bool matched) {
    return Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          const Text('Total Weightage Added:',
              style: TextStyle(
                  fontSize: 12.5,
                  fontWeight: FontWeight.bold,
                  color: Colors.grey)),
          Container(
            ...
            decoration: BoxDecoration(
              color:
                  matched ? const Color(0xFFD1FAE5) : const Color(0xFFFEF3C7),
            ...
            child: Text(
              ...
              style: TextStyle(
                fontWeight: FontWeight.w800,
                color:
                    matched ? const Color(0xFF065F46) : const Color(0xFFD97706),
                fontSize: 13,
              ),
            ),
          ),
          ...
          children: [
            ...
            if (options.isNotEmpty) ...[
              ...
              ...options.asMap().entries.map((entry) {
                ...
                question['correct_answer'].toString().isNotEmpty) ...[
              const SizedBox(height: 12),
              Text(
        ...
        children: [
          ...
          Expanded(
            child: ElevatedButton(
              ...
              child: Text(
                  marksMatched ? 'Proceed to Assign' : 'Assign (Incomplete)'),
            ),
          ),
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            ListTile(
              ...
              subtitle: const Text(
                  'Write a new question specifically for this exam.'),
              onTap: () {
            ...
            ListTile(
              ...
              subtitle: const Text(
                  'Automatically pick random questions matching the subject.'),
              onTap: () {
                Navigator.pop(context);
                _handleAutoGenerate();
              },
  ...
  Future<void> _handleAutoGenerate() async {
    ...
    try {
      ...
      final bankQuestions =
          await _apiService.getQuestionBank(subject: _exam!.subject);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content:
              Text('🎉 Auto-imported $addedCount questions matching syllabus!'),
  ...
  Future<void> _showQuestionBankSelection() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Failed to load bank: $e')));
    }
    ...
    if (bankQuestions.isEmpty) {
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(
              'There are no saved questions for subject "${_exam!.subject}". Please add questions to the Question Bank first.'),
          actions: [
            TextButton(
  ...
  void _showQuestionFormDialog({Map<String, dynamic>? question}) {
    ...
    if (parsedOptions.isNotEmpty) {
      optionControllers =
          parsedOptions.map((o) => TextEditingController(text: o)).toList();
    } else {
      optionControllers = [
    ...
    if (qType == 'multi_correct' && rawCorrectMulti.isNotEmpty) {
      try {
        if (rawCorrectMulti.startsWith('[')) {
          multiCorrectAnswers = List<String>.from(rawCorrectMulti
              .replaceAll('[', '')
              .replaceAll(']', '')
              .replaceAll('"', '')
              .split(',')
          ...
          multiCorrectAnswers = rawCorrectMulti
              .split(',')
              .map((s) => s.trim())
              .where((s) => s.isNotEmpty)
              .toList();
                                    ...
                                    if (qType == 'mcq' ||
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          Text(
                                            qType == 'multi_correct'
                                                ? 'Options & Correct Answers (tick all correct)'
                                                : 'Configure Options',
                                            style: const TextStyle(
                                                fontWeight: FontWeight.bold,
                                                fontSize: 13,
                                        ...
                                        if (formKey.currentState!.validate()) {
                                          ...
                                          try {
                                            if (question != null) {
                                              ...
                                              await _apiService.addExamQuestion(
                                                  _exam!.id, body);
                                              ScaffoldMessenger.of(context)
                                                  .showSnackBar(const SnackBar(
                                                      content: Text(
                              ...
                              onPressed: () async {
                                ...
                                try {
                                  final bytes = await _apiService
                                      .downloadExamPaperPdf(_exam!.id);
                                  final cleanTitle = _exam!.title
                                      .replaceAll(RegExp(r'\s+'), '_');
                                  final filename =
                                      'QuestionPaper_$cleanTitle.pdf';
                              ...
                              children: [
                                ...
                                if (_examQuestions.isEmpty)
                                  ...
                                  ..._examQuestions
                                    ...
                                    final qType = (q['question_type'] ?? 'mcq')
                                        .toString()
                                        .toLowerCase();
                                    ...
                                    if (optionsRaw is List) {
                                      options = optionsRaw
                                        ...
                                        optionsRaw.isNotEmpty) {
                                      try {
                                        options = List<String>.from(
                                            json.decode(optionsRaw));
                                      } catch (_) {}
                                    }
