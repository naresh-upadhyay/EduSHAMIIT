-- ============================================================================
-- Migration: 253_support_schedule_cancellation_scopes.sql
-- Description:
--   1. Ensures that cancelling or updating status on an occurrence ('this_event', 'following_events', 'entire_series')
--      properly populates status, cancellation_reason, cancelled_at, and cancelled_by.
--   2. Updates vehicle_trips status to 'cancelled' when schedule is cancelled.
--   3. Guarantees that overriding a single occurrence on a specific date never shifts the target date.
-- ============================================================================
...
BEGIN
    ...
    IF p_recurrence_scope = 'this_event' AND v_target_date IS NOT NULL THEN
        ...
        SET schedule_id = v_new_id,
            ...
            updated_at = NOW()
        WHERE (schedule_id = v_master_parent_id OR schedule_id = p_schedule_id OR schedule_id IN (SELECT id FROM public.schedules WHERE recurring_parent_id = v_master_parent_id))
          AND (schedule_instance_date = v_target_date OR start_date = v_target_date::TEXT);

        -- Exclude this date from master recurrence and current series
        PERFORM public.exclude_recurring_occurrence(v_master_parent_id, v_target_date);
        IF p_schedule_id != v_master_parent_id THEN
        ...
        WHERE (recurring_parent_id = v_master_parent_id OR recurring_parent_id = p_schedule_id OR id = p_schedule_id)
    ...
    ELSIF p_recurrence_scope IN ('this_and_following', 'following_events') AND v_target_date IS NOT NULL THEN
        ...
        UPDATE public.schedules
        SET deleted_at = NOW(), status = 'cancelled', updated_at = NOW()
        WHERE (recurring_parent_id = v_master_parent_id OR recurring_parent_id = p_schedule_id)
          AND (original_instance_date >= v_target_date OR DATE(start_time AT TIME ZONE v_tz) >= v_target_date)
          AND (recurrence_exception_type = 'override' OR is_recurring = FALSE)
          AND id != v_new_id;
    ...
    ELSE
        ...
        -- If status is cancelled, also update all child overrides and trips
        IF v_status = 'cancelled' THEN
            ...
            WHERE schedule_id = v_master_parent_id
               OR schedule_id IN (SELECT id FROM public.schedules WHERE recurring_parent_id = v_master_parent_id);
        END IF;
