-- ============================================================================
-- Migration: 256_fix_recurrence_entire_series_anchor_and_exceptions.sql
-- Description:
--   1. When updating a recurring series with 'entire_series', preserves the master
--      series start date (e.g. Aug 10) so earlier days (Aug 10) do NOT disappear,
--      while updating time-of-day, duration, metadata, and recurrence rules.
--   2. Ensures deleted occurrences in `schedule_recurrence.exceptions` are ALWAYS
--      preserved when editing 'entire_series' so deleted days STAY deleted.
--   3. Properly moves single instances on date change ('this_event') without leaving
--      a duplicate copy on the original date.
-- ============================================================================
...
BEGIN
    ...
    ELSE
        ...
        IF p_data->>'start_time' IS NOT NULL AND p_data->>'start_time' != '' THEN
            ...
            -- Preserve master series start date and update wall-clock time of day!
