...
BEGIN
  ...
  SELECT COALESCE(AVG(r.marks_obtained / r.max_marks * 100), 0) INTO v_avg_score FROM results r JOIN profiles p ON r.student_id = p.id WHERE p.school_id = p_school_id AND p.class = p_class;
