-- ============================================================================
-- Migration: 240_fix_create_recurrence_guard.sql
-- Description: Guard schedule_recurrence insert in fn_create_schedule against NULL frequency
-- ============================================================================
...
BEGIN
    ...
    v_freq := COALESCE(p_data->'recurrence'->>'frequency', p_data->>'frequency', 'none');
    v_is_rec := COALESCE((p_data->>'is_recurring')::BOOLEAN, FALSE) AND (v_freq != 'none');
    ...
    IF v_is_rec THEN
        INSERT INTO public.schedule_recurrence (
            ...
            v_freq,
            COALESCE((p_data->'recurrence'->>'interval')::INT, (p_data->>'interval')::INT, 1),
            COALESCE(p_data->'recurrence'->'days_of_week', p_data->'days_of_week', '[]'::jsonb),
            (COALESCE(p_data->'recurrence'->>'day_of_month', p_data->>'day_of_month'))::INT,
            (COALESCE(p_data->'recurrence'->>'month_of_year', p_data->>'month_of_year'))::INT,
            COALESCE(p_data->'recurrence'->>'end_type', p_data->>'end_type', 'never'),
            (COALESCE(p_data->'recurrence'->>'end_count', p_data->>'end_count'))::INT,
            (COALESCE(p_data->'recurrence'->>'end_date', p_data->>'end_date'))::DATE,
            '[]'::jsonb, NOW()
        );
    END IF;
