-- ============================================================================
-- Migration 311: Add Billable Days to Leave Applications & Synchronize Balances
-- Fixes leave deduction engine, holiday exclusion, overlap exclusion, and balance synchronization
-- ============================================================================

-- 1. Add billable_days, holidays_count, overlap_days_count to leave_applications
ALTER TABLE public.leave_applications 
ADD COLUMN IF NOT EXISTS billable_days NUMERIC(5, 1) DEFAULT NULL,
ADD COLUMN IF NOT EXISTS holidays_count INT DEFAULT 0,
ADD COLUMN IF NOT EXISTS overlap_days_count INT DEFAULT 0;

-- 2. Populate billable_days for existing records where NULL
UPDATE public.leave_applications
SET billable_days = CASE
    WHEN half_day_type IN ('FIRST_HALF', 'SECOND_HALF') THEN 0.5
    ELSE (end_date - start_date + 1)::NUMERIC(5, 1)
END
WHERE billable_days IS NULL;

-- 3. Comprehensive fn_apply_leave_request with balance guard and exact billable_days persistence
...
BEGIN
    ...
    -- 6. Find Leave Type ID & Check Balance
    ...
    -- 7. Insert Leave Application with exact billable_days
...
-- 4. Update fn_process_leave_action to use billable_days
...
BEGIN
    ...
    -- Use stored billable_days or calculate fallback
    v_days := COALESCE(
        v_leave.billable_days,
        CASE
            WHEN v_leave.half_day_type IN ('FIRST_HALF', 'SECOND_HALF') THEN 0.5
            ELSE (v_leave.end_date - v_leave.start_date + 1)::NUMERIC(5, 1)
        END
    );
    ...
    -- Adjust Balances accurately using billable_days
...
-- 5. Drop and recreate fn_get_leave_dashboard_and_requests
DROP FUNCTION IF EXISTS public.fn_get_leave_dashboard_and_requests(uuid,uuid,character varying,character varying,character varying,character varying,character varying,date,date,integer,integer,uuid);
DROP FUNCTION IF EXISTS public.fn_get_leave_dashboard_and_requests(uuid,character varying,character varying,character varying,character varying,character varying,date,date,integer,integer,uuid);
...
BEGIN
    ...
    -- 4. Balance Summary (Scoped to p_user_id)
...
-- 6. Self-Healing Stored Procedure to recalculate accurate leave_balances for all users
CREATE OR REPLACE FUNCTION public.fn_sync_all_leave_balances()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
...
BEGIN
    FOR v_rec IN SELECT DISTINCT user_id, leave_type_id, academic_year FROM public.leave_balances LOOP
        -- Sum approved billable days
        SELECT COALESCE(SUM(COALESCE(la.billable_days, CASE WHEN la.half_day_type IN ('FIRST_HALF', 'SECOND_HALF') THEN 0.5 ELSE (la.end_date - la.start_date + 1)::NUMERIC(5, 1) END)), 0.0)
        INTO v_used
        FROM public.leave_applications la
        WHERE la.applicant_id = v_rec.user_id
          AND la.leave_type_id = v_rec.leave_type_id
          AND LOWER(la.status) = 'approved';

        -- Sum pending billable days
        SELECT COALESCE(SUM(COALESCE(la.billable_days, CASE WHEN la.half_day_type IN ('FIRST_HALF', 'SECOND_HALF') THEN 0.5 ELSE (la.end_date - la.start_date + 1)::NUMERIC(5, 1) END)), 0.0)
        INTO v_pending
        FROM public.leave_applications la
        WHERE la.applicant_id = v_rec.user_id
        ...
        SET used_days = v_used,
            ...
            updated_at = NOW()
        WHERE user_id = v_rec.user_id AND leave_type_id = v_rec.leave_type_id AND academic_year = v_rec.academic_year;
    END LOOP;

    RETURN jsonb_build_object('success', TRUE, 'message', 'All leave balances synchronized successfully');
END;
$function$;

-- Run synchronization now
SELECT public.fn_sync_all_leave_balances();
