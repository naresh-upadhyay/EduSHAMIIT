-- ============================================================================
-- Migration 284: Fix Class Teacher Consistency
-- Description:
-- 1. Cleans up stale orphaned class-level assignments where section assignments exist.
-- 2. Drops previous overloaded versions of fn_get_academic_class_detail.
-- 3. Updates fn_get_academic_class_detail to resolve the primary class teacher
--    consistently, prioritizing the section class teacher when sections exist.
-- ============================================================================

-- Step 1: Clean up stale class-level assignments when section-level assignments exist
DELETE FROM public.class_teacher_assignments cta_outer
WHERE cta_outer.section_id IS NULL
  AND EXISTS (
      SELECT 1 
      FROM public.class_teacher_assignments cta_inner 
      WHERE cta_inner.class_id = cta_outer.class_id 
        AND cta_inner.section_id IS NOT NULL
  );

-- Step 2: Drop previous overloaded versions
...
-- Step 3: Create fn_get_academic_class_detail
...
BEGIN
    ...
    FROM (
        SELECT 
            ...
            sub.is_optional,
            COALESCE(
                string_agg(DISTINCT sec.name, ', ' ORDER BY sec.name ASC),
                'All Sections'
            ) AS section_names
        FROM public.class_subject_assignments csa
    ...
    RETURN jsonb_build_object(
        ...
        'data', jsonb_build_object(
            ...
            'sections_count', v_total_sections,
