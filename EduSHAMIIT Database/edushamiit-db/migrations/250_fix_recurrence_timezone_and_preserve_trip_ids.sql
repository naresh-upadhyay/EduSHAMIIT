-- ============================================================================
-- Migration: 250_fix_recurrence_timezone_and_preserve_trip_ids.sql
-- Description:
--   1. Fixes trip matching in fn_sync_schedule_vehicle_trips across all schedule
--      ancestry/overrides so trip_id NEVER changes on edit.
--   2. In fn_update_schedule: Sets recurring_parent_id on split series so triggers
--      seamlessly adopt and preserve existing trip IDs.
--   3. Cleans up duplicate vehicle_trips rows for identical dates on the same schedule family.
-- ============================================================================
...
BEGIN
    ...
    IF NOT COALESCE(v_sched.is_recurring, FALSE) THEN
        ...
        -- Check existing trip matching this schedule, parent, or siblings
        SELECT id INTO v_existing_id
        FROM public.vehicle_trips
        WHERE (
            schedule_id = p_schedule_id
            OR (v_parent_id IS NOT NULL AND schedule_id = v_parent_id)
            OR schedule_id IN (SELECT id FROM public.schedules WHERE id = p_schedule_id OR id = v_parent_id OR recurring_parent_id = p_schedule_id OR (v_parent_id IS NOT NULL AND recurring_parent_id = v_parent_id))
        )
        AND (schedule_instance_date = v_cur_date OR start_date = v_cur_date::TEXT OR (scheduled_start AT TIME ZONE v_tz)::DATE = v_cur_date)
        ORDER BY CASE WHEN schedule_id = p_schedule_id THEN 1 ELSE 2 END, CASE WHEN status IN ('in_progress', 'paused') THEN 1 WHEN status = 'completed' THEN 2 ELSE 3 END, updated_at DESC
        LIMIT 1;
    ...
    WHILE v_cur_date <= v_end_horizon LOOP
        ...
        IF v_should_create THEN
            ...
            -- Check existing trip row for this specific occurrence date across ancestry
            SELECT id INTO v_existing_id
            FROM public.vehicle_trips
            WHERE (
                schedule_id = p_schedule_id
            ...
            -- Delete any duplicates on this same date for this schedule
            DELETE FROM public.vehicle_trips
            WHERE schedule_id = p_schedule_id
              AND (schedule_instance_date = v_cur_date OR start_date = v_cur_date::TEXT)
              AND id != COALESCE(v_existing_id, (SELECT id FROM public.vehicle_trips WHERE schedule_id = p_schedule_id AND (schedule_instance_date = v_cur_date OR start_date = v_cur_date::TEXT) ORDER BY CASE WHEN status IN ('in_progress', 'paused') THEN 1 WHEN status = 'completed' THEN 2 ELSE 3 END, created_at DESC LIMIT 1));
