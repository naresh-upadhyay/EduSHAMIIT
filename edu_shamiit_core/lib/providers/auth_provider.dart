...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  static void Function(String userId, String baseUrl)? onLogin;
    ...
    try {
      ...
      if (token != null && roleStr != null) {
        ...
        if (userId.isNotEmpty) {
          onLogin?.call(userId, AppConfig.baseUrl);
        }
        return;
      }
    } catch (e) {
  ...
  Future<bool> signIn({
    ...
    try {
      ...
      if (response.statusCode == 200 && body['success'] == true) {
        ...
        // Initialize CallService so incoming calls work from this login via callback
  ...
  Future<void> signOut() async {
    // Clear API cache
    ApiService().clearCache();
    // Dispose CallService incoming listener via callback
    onLogout?.call();
